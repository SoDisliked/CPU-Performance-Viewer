...
namespace PerfView
{
    ...
    /// </summary>
    public partial class SelectProcess : WindowsBase
    {
        public SelectProcess(Window parentWindow, IEnumerable<IProcess> processes, TimeSpan maxLifetime, Action<List<IProcess>> action, bool hasAllProc = false) :
    base(parentWindow)
        {
            m_action = action;
            ...
            if (!hasAllProc)
            {
                AllProcsButton.Visibility = System.Window.Visibility.Hidden;
            }
            ...
            // Set selection point to the 1st process
        ...
        private static int FindNextWithPrefix(List<IProcess> processes, int start, string prefix)
        {
            ...
            // adds the prefix configurator for the next command 
        ...
        internal void OKClicked(object sender, RoutedEventArgs e)
        {
            ...
            {
                OKButton.ToolTip = "Select the button to make selection";
        ...
        private void GridKeyDownHandder(object sender, KeyEventArgs e)
        {
            if (e.Key == Key.Return)
            {
                OKClicked(null, null);
            ...
            {
                // Selected key will be specified.
